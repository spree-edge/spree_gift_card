...
describe Spree::GiftCard, type: :model do
  ...
  # Constants
  it { expect(Spree::GiftCard::UNACTIVATABLE_ORDER_STATES).to eq(%w[complete awaiting_return returned]) }
  ...
  context '#activatable?' do
    ...
    it 'expects to be activatable if created before order, has current value, and order state valid' do
      expect(gift_card.order_activatable?(mock_model(Spree::Order, state: 'cart',
                                                                   created_at: (gift_card.created_at + 1.second)))).to be_truthy
    end

    it 'expects not to be activatable if created after order' do
      expect(gift_card.order_activatable?(mock_model(Spree::Order, state: 'cart',
                                                                   created_at: (gift_card.created_at - 1.second)))).to be_falsey
    end
    ...
    it 'expects not to be activatable if invalid order state' do
      expect(gift_card.order_activatable?(mock_model(Spree::Order, state: 'complete',
                                                                   created_at: (gift_card.created_at + 1.second)))).to be_falsey
    end
  end
  ...
  describe 'authorize' do
    ...
    context 'when already authorized' do
      ...
      before do
        gift_card.transactions.create!(action: Spree::GiftCard::AUTHORIZE_ACTION, amount: authorized_amount,
      ...
      it 'expects not to re-authorize given amount' do
        ...
        end.to_not change { gift_card.authorized_amount }
      end
    end
    ...
    context 'when authorization amount is less than amount remaining' do
      ...
      it 'expects to authorize given amount' do
        expect do
          gift_card_authorize
        end.to change { gift_card.authorized_amount }.by(authorized_amount)
      end

      it 'expects to create an authorize transaction for the given amount' do
        gift_card_authorize
        expect(gift_card.transactions.where(action: Spree::GiftCard::AUTHORIZE_ACTION, amount: authorized_amount,
                                            order: order).exists?).to be_truthy
      end
    end
    ...
    context 'when authorization amount is greater than amount remaining' do
      ...
      it 'expects not to authorize given amount' do
        expect do
          gift_card_authorize
        end.not_to change { gift_card.authorized_amount }
      end

      it 'expects not to create an authorize transaction for the given amount' do
        gift_card_authorize
        expect(gift_card.transactions.where(action: Spree::GiftCard::AUTHORIZE_ACTION, amount: authorized_amount,
                                            order: order).exists?).to be_falsey
      end
    end
  end
  ...
  describe 'capture' do
    ...
    let(:authorized_amount) { 13.0 }
    let(:auth_code) do
      gift_card.transactions.find_by(action: Spree::GiftCard::AUTHORIZE_ACTION,
                                     amount: authorized_amount).authorization_code
    end
    let(:order) { create(:order) }
    ...
    context 'when capture amount is less than authorized_amount' do
      ...
      it 'expects to remove amount from authorization' do
        expect do
          capture_payment
        end.to change { gift_card.authorized_amount }.by(-capture_amount)
      end

      it 'expects to remove amount from current_value' do
        expect do
          capture_payment
        end.to change { gift_card.current_value }.by(-capture_amount)
      end

      it 'expects to create a capture transaction for the given amount' do
        capture_payment
        expect(gift_card.transactions.where(action: Spree::GiftCard::CAPTURE_ACTION, amount: capture_amount,
                                            authorization_code: auth_code, order: order).exists?).to be_truthy
      end
    end
    ...
    context 'when capture amount is greater than authorized_amount' do
      ...
      it 'expects not to remove amount from current_value' do
        expect do
          capture_payment
        end.to_not change { gift_card.current_value }
      end

      it 'expects not to create a capture transaction for the given amount' do
        capture_payment
        expect(gift_card.transactions.where(action: Spree::GiftCard::CAPTURE_ACTION, amount: capture_amount,
                                            authorization_code: auth_code, order: order).exists?).to be_falsey
      end
    end
  ...
  describe 'void' do
    ...
    context 'when auth transaction present with auth code' do
      ...
      it 'expects to create a void transaction for the given amount' do
        void_payment
        expect(gift_card.transactions.where(action: Spree::GiftCard::VOID_ACTION, amount: authorized_amount,
                                            authorization_code: auth_code, order: order).exists?).to be_truthy
      end
    end
  ...
  describe 'credit' do
    ...
    let(:auth_code) do
      gift_card.transactions.find_by(action: Spree::GiftCard::AUTHORIZE_ACTION,
                                     amount: captured_amount).authorization_code
    end
    let(:order) { create(:order) }
    ...
    context 'when credit amount is less than captured amount' do
      ...
      it 'expects to add amount to current_value' do
        expect do
          credit_payment
        end.to change { gift_card.current_value }.by(credit_amount)
      end

      it 'expects to create a credit transaction for the given amount' do
        credit_payment
        expect(gift_card.transactions.where(action: Spree::GiftCard::CREDIT_ACTION, amount: credit_amount,
                                            authorization_code: auth_code, order: order).exists?).to be_truthy
      end
    end
